const log1Element = document.getElementById("log1") as HTMLDivElement;
const log2Element = document.getElementById("log2") as HTMLDivElement;
...
function updateRobot() {
    ...
    /* -------------------------------------------------------------------------- */
    function _calculateDistance([x1, y1]: number[], [x2, y2]: number[]): number {
        return Math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2);
    }
    function _calculateTipPosition(armAngles: number[]): [number, number] {
...
/* -------------------------------------------------------------------------- */
function _xyToCanvasSpace(x: number, y: number): [number, number] {
    return [x + width / 2, -y + height / 2];
}
function _drawCentredRect(x: number, y: number, w: number, h: number, colour: string) {
    ctx.fillStyle = colour;
    const [cx, cy] = _xyToCanvasSpace(x, y);
    ctx.fillRect(cx - w / 2, cy - h / 2, w, h);
}
function _drawFromToRect(x1: number, y1: number, x2: number, y2: number, colour: string) {
    ctx.fillStyle = colour;
    const [cx1, cy1] = _xyToCanvasSpace(x1, y1);
    const [cx2, cy2] = _xyToCanvasSpace(x2, y2);
    ctx.fillRect(cx1, cy1, cx2 - cx1, cy2 - cy1);
}
function _drawCircle(x: number, y: number, r: number, colour: string) {
    ctx.fillStyle = colour;
    ctx.beginPath();
    const [cx, cy] = _xyToCanvasSpace(x, y);
...
function _drawLine(x1: number, y1: number, x2: number, y2: number, thickness: number, colour: string) {
